...
\begin{itemize}
    ...
    \item \textbf{user\_info}: demographic and factual attributes (populated \textit{only} at the root node; e.g., ``24-year-old male, works in distribution, frequently active near Gangnam Station and Hannam-dong''; all sub-nodes are constrained to empty lists),
    \item \textbf{summary}: a natural-language paragraph summarizing the user's preferences and behavioral patterns for this category, integrating both explicit and implicit signals (e.g., for the Korean Food node: preferences for curry-based dishes and bold flavors, inferred from repeated visits and a review stating ``this place is legendary---I love the curry''),
    ...
    \item \textbf{recently\_updated}: the date of the most recent content update (e.g., the Korean Food note advances from 2025/02/19 to 2025/02/25 after new activity arrives, while the Tennis note, last updated on 2025/10/23, remains unchanged).
\end{itemize}

As illustrated in Figure~\ref{fig:workflow}, a user's initial memory may contain nodes such as \texttt{User $>$ Dining $>$ Korean Food} and \texttt{User $>$ Sports $>$ Tennis}, each holding independently tracked preference notes. When new activity arrives---for example, a curry restaurant review and visit on 2025/02/25---the forward pass selectively updates only the affected nodes (root and Korean Food), leaving unrelated nodes (Tennis) untouched. The backward pass subsequently verifies structural integrity and removes leaf nodes that have not been updated within a configurable staleness window.
...
\begin{figure}[t]
  ...
  \caption{Concrete example of Memory Agent operation. \textit{Left}: initial hierarchical note memory with two Level-2 domains (Dining, Sports) and three leaf nodes (Korean Food, Chinese Food, Tennis), each holding a note with a timestamp and summary. \textit{Center}: the forward aggregation pass triggered by new 2025/02/25 activity (curry restaurant review, visit, and search)---only the root and Korean Food nodes are updated; Tennis is untouched. \textit{Right}: the backward consolidation pass restructures the tree, elevates common traits to parent nodes, and removes leaf nodes that have not received any update for $N$ months.}
  \label{fig:workflow}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Fixed-p}: PersonaMem-v2-style~\cite{personamem-v2} three-level tree with predefined fixed slots (tests dynamic routing value).
